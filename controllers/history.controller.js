    ...
    try {
        ...
        if (!findVideo) {
            ...
            try {
                ...
                return res.status(500).json({ message: "Couldn't fetch transcript", error: error.message });
            }
            
            if (!transcript || !Array.isArray(transcript)) {
                return res.status(500).json({ message: "Transcript format is invalid or empty" });
            }
            ...
            let summaryORG = await createChatWithGoogle(textes.join(' '), ask);
            let summary = await translateResult(summaryORG, lang);

            if (summaryORG === "" || summary === "") {
                return res.status(500).json({ message: "AI couldn't generate a summary. Please try again later!" });
            }
